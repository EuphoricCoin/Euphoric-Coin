...
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // EuphoricCoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "EuphoricCoin Signed Message:\n";
...
{
    // EuphoricCoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 5275200); // EuphoricCoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 2 * 24 * 60 * 60; // EuphoricCoin: 2 days
static const int64 nTargetSpacing = 2 * 60; // EuphoricCoin: 2 minutes or 120 Secs
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // EuphoricCoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // EuphoricCoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // EuphoricCoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Snow turns Atlanta into 'zombie movie' Drivers camped out on frozen roads. Kids sleeping on cold school floors. ";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        block.nVersion = 1;
        block.nTime    = 1391012260;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084524493;

        if (fTestNet)
        {
            block.nTime    = 1391012261;
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // EuphoricCoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("EuphoricCoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("EuphoricCoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static EuphoricCoinMiner(CWallet *pwallet)
{
    printf("EuphoricCoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("EuphoricCoin-miner");
    ...
    try { loop {
        ...
        printf("Running EuphoricCoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("EuphoricCoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&EuphoricCoinMiner, pwallet));
}
